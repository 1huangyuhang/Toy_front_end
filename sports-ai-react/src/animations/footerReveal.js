...
export function setupFooterReveal({ footerEl, contentEl, spacerEl, options = {} }) {
  ...
  function tick() {
    raf = 0
    const footerH = measureFooterHeight()
    ...
    footerEl.style.setProperty('--footer-reveal-h', `${Math.round(footerH * eased)}px`)
    // 不做渐隐：只用遮罩高度控制“揭开”，不改 opacity/display

    // 防止 footer 覆盖内容交互：接近底部才允许点击
    footerEl.style.pointerEvents = eased > 0.1 ? 'auto' : 'none'

    // 当内容到底部时，避免 footer 与底部间空隙闪动
    footerEl.style.setProperty('--footer-pad', `${Math.max(0, footerH - remaining)}px`)
  }
